<template>
    <div>
        <el-table :data="this.rows" border style="width: 100%">
            ...
            <el-table-column prop="InventorymoveNo" label="移库编号"></el-table-column>
            <el-table-column prop="SourceStoragerackId" label="原货架编号"></el-table-column>
            <el-table-column prop="AimStoragerackId" label="目标货架编号"></el-table-column>
...
export default {
    ...
    methods: {
        ...
        handleEdit(index, row) {
            // 触发编辑承运商事件，传递当前行数据
            this.$emit('editInventorymove', row);
        },
        handleDelete(index, row) {
            // 触发删除承运商事件，传递当前行数据
            this.$emit('deleteInventorymove', row);
        },
