<template>
  <div>
    <Search @search="handleSearch" @addDept="showAddDeptDialog" :currentPage="currentPage" @update-page="updatePage" />
    ...
    <AddEditDialog :visible.sync="addEditDialogVisible" :title="dialogTitle" :dept="selectedDept"
     :formFields="formFields" @confirmAction="confirmAddEditDept"
...
export default {
  ...
  methods: {
    init() {
      ...
      this.$axios.post(this.$httpUrl + '/Dept/GetPageList', UserFormData)
        .then(response => {
    ...
    handleSearch(searchData) {
      ...
      this.parmas.search = searchData.deptName;
      this.init(); // 重新获取数据
    },
    showAddDeptDialog() {
      // 显示新增部门对话框的逻辑
    ...
    handleDeleteDept(deptData) {
      ...
      UserFormData.append("userId", this.$store.state.user.UserId);
      UserFormData.append("dept", JSON.stringify(deptDto));
      this.$axios.post(this.$httpUrl + '/Dept/DeleteDept', UserFormData)
        .then(response => {
          const data = response.data;
          if (data.Item1) {
            this.init();
